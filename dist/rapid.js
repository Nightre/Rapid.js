var ScaleRadio;
(function (ScaleRadio) {
    ScaleRadio["KEEP"] = "keep";
    ScaleRadio["KEEP_H"] = "keep_h";
    ScaleRadio["KEEP_W"] = "keep_w";
    ScaleRadio["IGNORE"] = "ignore";
    ScaleRadio["EXPAND"] = "expand";
...
class Color {
     ...
     */
    subtract(color) {
        return new Color(Math.max(0, this.r - color.r), // Clamp to 0
...
class TileMapRender {
     ...
     * @private
     */
    getYSortRow(ySortCallbacks, height) {
        const rows = new Map();
        if (!ySortCallbacks) {
            return rows;
        }
    ...
    getTileData(tileSet, options) {
        ...
        const startTile = new Vec2(Math.floor(minX / width) - errorX, Math.floor(minY / height) - errorY);
        const endTile = new Vec2(Math.ceil(maxX / width) + errorX, Math.ceil(maxY / height) + errorY);
    ...
    renderLayer(data, options) {
        ...
        const { startTile, viewportWidth, viewportHeight, height } = this.getTileData(tileSet, options);
        // --- 这里是改动点 ---
        ...
        for (let y = 0; y < viewportHeight; y++) {
            ...
            const currentRow = ySortRowsMap.get(mapY) ?? [];
            // 后续逻辑保持不变...
...
Easing.EaseInOutQuad = (t) => t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t;
Easing.EaseInCubic = (t) => t * t * t;
Easing.EaseOutCubic = (t) => (--t) * t * t + 1;
Easing.EaseInOutCubic = (t) => t < 0.5 ? 4 * t * t * t : (t - 1) * (2 * t - 2) * (2 * t - 2) + 1;
Easing.EaseInQuart = (t) => t * t * t * t;
Easing.EaseOutQuart = (t) => 1 - (--t) * t * t * t;
Easing.EaseInOutQuart = (t) => t < 0.5 ? 8 * t * t * t * t : 1 - 8 * (--t) * t * t * t;
/**
...
class ParticleEmitter {
    ...
    constructor(rapid, options) {
        ...
        this.emitting = false;
        this.emitTimer = 0;
        this.emitRate = DEFAULT_EMIT_RATE; // In continuous mode: particles/sec. In burst mode: particles/burst.
        this.emitTime = DEFAULT_EMIT_TIME; // Time interval between emissions (0 for continuous)
...
class Rapid {
    ...
    endFBO() {
        if (this.currentFBO.length > 0) {
            ...
            fbo.unbind();
            this.clearTextureUnit();
            this.resizeSize(this.logicWidth, this.logicHeight, this.physicsWidth, this.physicsHeight);
...
class InputManager {
    ...
    constructor(rapid) {
        ...
        this.keysDownLastFrame = new Set();
        ...
        this.buttonsDownLastFrame = new Set();
        this.rapid = rapid;
...
class Camera extends Entity {
    ...
    updateTransform() {
        const transform = this.transform;
        transform.identity();
        const centeredPosition = new Vec2(-this.rapid.logicWidth, -this.rapid.logicHeight).divide(2);
...
class Tilemap extends Entity {
    ...
    constructor(game, options) {
        super(game, options);
        this.error = new Vec2(Tilemap.DEFAULT_ERROR);
        this.data = [[]];
        this.ySortCallback = [];
        if (options.error) {
            if (typeof options.error === "number") {
...
Tilemap.DEFAULT_ERROR = 0;
Tilemap.EMPTY_TILE = -1;
/**
...
export { ArrayType, BaseTexture, BlendMode, Camera, Color, DynamicArrayBuffer, Entity, FrameBufferObject, GLShader, Game, InputManager, LineTextureMode, MaskType, MathUtils, MatrixStack, ParticleEmitter, ParticleShape, Random, Rapid, SCALEFACTOR, ScaleRadio, Scene, ShaderType, Text, Texture, TextureCache, TextureWrapMode, TileMapRender, TileSet, Tilemap, TilemapShape, Uniform, Vec2, WebglBufferArray, WebglElementBufferArray, graphicAttributes, spriteAttributes };
