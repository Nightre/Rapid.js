...
- `sampleTexture(uv)`：采样当前 Texture，并乘上 `vColor`。
- `sampleClampTexture(uv)`：采样当前 Texture，超出 `vUVRect` 时返回透明。
- `sampleTextureLocal(uv)`：用 0 到 1 的局部坐标采样当前 Texture 区域。
...
const outline = new CustomGlShader(rapid, vs, outlineFs, 0, {
  ...
  uTexel: [1 / texture.base!.width, 1 / texture.base!.height],
}).setPadding(1);
